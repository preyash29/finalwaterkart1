...
import { distributorMyvendors, removeVendors } from '../../../actions'
...
export default function vendors ({ navigation }) {
  ...
  const _removeVendor = async (data) => {
    dispatch(removeVendors({ vendor_id: data.id }))
  }
  ...
  const renderItem = ({ item }) => {
    return (
      ...
      >
        ...
        {
          (item.status === 1 && item.request_type === 0) || (item.status === 2 && item.request_type === 1) ? (
            <TouchableOpacity
              onPress={() => _removeVendor(item)}
              style={styles.btnAction}
            >
              <Text style={[styles.otherText, { color: colors.white, marginTop: 3 }]}>Remove vendor</Text>
            </TouchableOpacity>
          ) : null
        }
        {
          item.status === 0 && item.request_type === 1 && (
            <Text style={[styles.otherText, { color: colors.blueLight, textAlign: 'right', marginRight: 10 }]}>Requested sent to remove</Text>
          )
        }
        {/* {
          item.status === 1 && (
        ...
        {
          item.status === 2 && (
            <Text style={[styles.otherText, { color: colors.blueLight, textAlign: 'right', marginRight: 10 }]}>Requested to add</Text>
          )
        } */}
      </TouchableOpacity>
    )
