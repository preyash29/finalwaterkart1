...
class AllTransactions extends React.Component {
    ...
    initData = async () => {
        let res = await this.props.getWalletTransactions()
        if (res.status) {
            let data = []
    ...
    renderItem = ({ item }) => {
        return (
            <View style={styles.item}>
                <View>
                <Text style={styles.orderText}>{item.type === 2 ? item.title : 'Order #' + item.order_id}</Text>
                    <Text style={styles.orderTimeText}>{moment(Date(item.created_at)).format('hh:mm A')}, {item.type === 1 ? 'Credit' : 'Debit'}</Text>
                </View>
                <Text style={styles.orderText}>{item.type === 1 ? '-' : '+'} Rs. {item.amount}</Text>
            </View>
        )
    }
}
